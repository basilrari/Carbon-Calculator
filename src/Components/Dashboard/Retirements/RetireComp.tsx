...
const RetirementComp: React.FC<{ quantity: number; poolName: string }> = ({ quantity, poolName }) => {
  return (
    <div className="flex items-center justify-between p-6 border rounded-lg shadow-md bg-white w-full mx-auto">
      <div className="flex items-center space-x-4">
        ...
        />
        <div>
          <h5 className="text-sm font-medium text-gray-600">{poolName}</h5>
      ...
      <div className="flex space-x-4">
        <Link href="/decarb/retirements/retireassets">
          <MyButton variant="green" text="RETIRE" />
        </Link>
      </div>
    </div>
  );
