...
import { MdOutlineKeyboardArrowLeft, MdOutlineKeyboardArrowRight, MdClose, MdMenu } from "react-icons/md";
...
const Sidebar = () => {
  ...
  return (
    <>
      ...
      <div className={`hidden md:flex h-screen bg-[#F1F6F5] border-r border-gray-200 p-4 transition-all duration-300 ${isCollapsed ? "w-32" : "w-64"}`}>
        <div className="flex flex-col h-full">
          {/* Logo and Toggle Button */}
          <div className="flex items-center justify-between mb-10">
            <div className="flex items-center">
              <Image src="/images/decarblogo.png" alt="Logo" width={64} height={64} />
              <h1 className={`text-3xl font-bold text-[#2F4F4F] ml-2 ${isCollapsed ? "hidden" : ""}`}>Decarb</h1>
            </div>
            <button className="text-[#2F4F4F] bg-green-50 rounded-lg p-1.5 border-b border-black -mr-4" onClick={toggleSidebar}>
          ...
          <nav>
            <ul className="space-y-8">
              {navItems.map((item) => {
                ...
                return (
                  <li key={item.href}>
                    <Link href={item.href} className={`group flex items-center text-black rounded-md p-2 pl-5 hover:bg-[#9BC3BF] ${isActive ? "bg-[#9BC3BF]" : ""}`}>
                      <Image src={item.icon} alt={item.label} width={32} height={32} />
                      <span className={`font-semibold text-[#2F4F4F] text-lg ml-4 ${isCollapsed ? "hidden" : ""}`}>
                        {item.label}
                      </span>
                    </Link>
                  </li>
      ...
      {isMobileOpen && (
        <div className="fixed inset-0 z-50 bg-[#F1F6F5] p-6 flex flex-col h-screen">
          {/* Close Button */}
          <button className="absolute top-4 right-4 text-black p-2 rounded-lg" onClick={toggleMobileSidebar}>
            <MdClose size={28} />
          </button>
          ...
          <nav className="flex-1">
            <ul className="space-y-6">
              {navItems.map((item) => {
                ...
                return (
                  <li key={item.href}>
                    <Link href={item.href} className={`group flex items-center text-black rounded-md p-3 hover:bg-[#9BC3BF] ${isActive ? "bg-[#9BC3BF]" : ""}`} onClick={toggleMobileSidebar}>
                      <Image src={item.icon} alt={item.label} width={32} height={32} />
